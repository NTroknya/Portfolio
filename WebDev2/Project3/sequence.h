...
//	2.	All members function that access sequence items by position should throw an exception
...
public:
	...
	size_type size() const;							//returns size of the sequence

	void clear();									// clears the sequence returning it to the empty state
	void erase( size_type p, size_type n = 1 );	// deletes n number items starting at index position p 	
	void print();									//Helper function to assist with >> overload
